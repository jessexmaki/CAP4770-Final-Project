...
for subject in subject_list:
    img = nib.load(os.path.join(data_directory, f"{subject}.nii_seg_pve_1.nii.gz"))  # Adjusted filename format
    ...
    if demographic_data is None:
        print(f"Warning: No corresponding record found for image {subject}.nii_seg_pve_1.nii.gz")
        continue
    age = demographic_data['AGE']
    data_list.append(img_data)
    labels.append(age)
    print(f"Image {subject}.nii_seg_pve_1.nii.gz loaded successfully!")  
...
np.savez('/content/drive/My Drive/Final-Project/preprocessed_data1.npz', x_train=x_train, y_train=y_train, x_val=x_val, y_val=y_val, x_test=x_test, y_test=y_test)
